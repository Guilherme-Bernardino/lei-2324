...
@personTypeId int)
AS
DECLARE @hashedPass nvarchar(50)
BEGIN
	SET @hashedPass = [WWI].[dbo].[encryptPassword](@password)
	INSERT INTO [UserSchema].[Person](name, preferredName,email, password, photo, personTypeId)
	VALUES (@name, @preferredName, @email, @hashedPass, @photo, @personTypeId)
END
...
AS
BEGIN 
	INSERT INTO [SalesSchema].[Discount](discountPercentage, beginDateGDH, endDateGDH)
	VALUES (@discountPercentage, @beginDateGDH, @endDateGDH)
END
...
AS
BEGIN 
	INSERT INTO [SalesSchema].[Sale](idSale, deliveryDate, employeeId, customerId, invoiceId, cityId, notes)
	VALUES (@idSale, @deliveryDate, @employeeId, @customerId, @invoiceId, @cityId, @notes)
END
...
AS
BEGIN 
	INSERT INTO [SalesSchema].[SaleProduct](productId, saleId, quantity, description, totalExcludingTax, taxAmout, profit, totalIncludingTax, totalDryItens, totalChillerItens)
...
AS
BEGIN 
	UPDATE [SalesSchema].[SaleProduct] 
...
AS
BEGIN 
	DELETE FROM [SalesSchema].[SaleProduct] 
	WHERE idSaleProduct = @idSaleProduct
...
as
BEGIN
	IF(EXISTS (SELECT idSale from [SalesSchema].[Sale] as s join [SalesSchema].[SaleProduct] as c on (c.saleId = s.idSale) WHERE idSale = @idSale))
	BEGIN
		RETURN 0;
	END
	RETURN 1;
...
AS
BEGIN
	IF EXISTS(SELECT totalChillerItens FROM [WWI].[SalesSchema].[SaleProduct] as sp
		JOIN [WWI].[SalesSchema].[Sale] as s on s.idSale = sp.saleId
...
ON [UserSchema].[Person]
AFTER INSERT
AS
IF exists ( SELECT * FROM [UserSchema].[Person] t inner join inserted i on i.email=t.email and i.idPerson <> t.idPerson)
BEGIN
    ROLLBACK
...
as
BEGIN
DECLARE @idvalue int, @hashedPassword nvarchar(50)
	SET @hashedPassword = [WWI].[dbo].[encryptPassword](@password)
	IF(EXISTS(SELECT * FROM [UserSchema].[Person] as p
	where p.email = @email and p.password = @hashedPassword  and p.personTypeId = 2))
	BEGIN
		PRINT('Aunthentication valid!');
		SET @idvalue = (SELECT p.idPerson FROM [UserSchema].[Person] as p
						where p.email = @email and p.password = @hashedPassword and p.personTypeId = 2)
		INSERT INTO [UserSchema].[AuthenticationEvent] 
...
SET @idPerson = (SELECT idPerson FROM [UserSchema].[Person]
				WHERE email = @email)

	INSERT INTO [UserSchema].[TokenPassword]
