...
FROM [WWI].[dbo].[Customer] as c
JOIN [WWI].[dbo].[Category] as ca on ca.idCategory = c.categoryId
group by ca.name
...
FROM [WWI].[dbo].[Sale] as Sale
join [WWI].[dbo].[Employee] as Employee on Employee.idEmployee = Sale.employeeId
join [WWI].[dbo].[Person] as Person on Person.idPerson = Employee.personId
...
JOIN [WWI].[dbo].[Product] as p on p.idProduct = sp.productId
JOIN [WWI].[dbo].[Sale] as s on s.idSale = sp.saleId
JOIN [WWI].[dbo].[InvoiceCustomer] as ic on ic.saleId = s.idSale
JOIN [WWI].[dbo].[Invoice] as i on i.idInvoice = ic.invoiceId
JOIN [WWI].[dbo].[City] as c on c.idCity = i.cityId
